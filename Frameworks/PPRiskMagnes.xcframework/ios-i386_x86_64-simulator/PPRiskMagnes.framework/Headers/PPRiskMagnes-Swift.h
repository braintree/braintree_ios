...
@class UITouch;
@class UIEvent;
enum TouchState : NSInteger;

/// Magnes is PayPal’s mobile Risk SDK…
...
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_DEPRECATED_MSG("-init is unavailable");
/// Pass in optional parameters to setup the MagnesSDK
...
/// returns:
/// Void
- (void)collectTouchData:(NSSet<UITouch *> * _Nonnull)touches withEvent:(UIEvent * _Nullable)event forTouchState:(enum TouchState)touchState withPayPalClientMetadataId:(NSString * _Nonnull)cmid;
...
typedef SWIFT_ENUM(NSInteger, TouchState, open) {
  TouchStateTouchBegan = 0,
  TouchStateTouchMoved = 1,
  TouchStateTouchEnded = 2,
};
