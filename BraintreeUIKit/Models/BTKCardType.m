...
+ (instancetype)cardTypeForNumber:(NSString *)number {
    ...
    for (BTKCardType *cardType in [[self class] allCards]) {
...
+ (NSArray *)possibleCardTypesForNumber:(NSString *)number {
    number = [BTKUtil stripNonDigits:number];
    if (number.length == 0) {
...
- (NSString *)description {
    return [NSString stringWithFormat:@"BTKCardType %@", self.brand];
}
...
+ (NSUInteger)maxNumberLength {
    ...
    dispatch_once(&p, ^{
        for (BTKCardType *t in [self allCards]) {
...
+ (NSArray *)allCards
{
    ...
    dispatch_once(&p, ^{
        ...
        BTKCardType *visa = [[BTKCardType alloc] initWithBrand:BTKLocalizedString(CARD_TYPE_VISA) prefixes:@[@"4"]];
        BTKCardType *mastercard = [[BTKCardType alloc] initWithBrand:BTKLocalizedString(CARD_TYPE_MASTER_CARD)
                                                              prefixes:@[@"51", @"52", @"53", @"54", @"55"]];
        BTKCardType *discover = [[BTKCardType alloc] initWithBrand:BTKLocalizedString(CARD_TYPE_DISCOVER) prefixes:@[@"6011", @"65", @"644", @"645", @"646", @"647", @"648", @"649"]];
        BTKCardType *jcb = [[BTKCardType alloc] initWithBrand:BTKLocalizedString(CARD_TYPE_JCB) prefixes:@[@"35"]];

        BTKCardType *amex = [[BTKCardType alloc] initWithBrand:BTKLocalizedString(CARD_TYPE_AMERICAN_EXPRESS)
                                                        prefixes:@[@"34", @"37"]
                                              validNumberLengths:[NSIndexSet indexSetWithIndex:15]
                                                  validCvvLength:4
                                                    formatSpaces:@[@4, @10]];

        BTKCardType *dinersClub = [[BTKCardType alloc] initWithBrand:BTKLocalizedString(CARD_TYPE_DINERS_CLUB)
                                                              prefixes:@[@"36", @"38", @"300", @"301", @"302", @"303", @"304", @"305"]
                                                    validNumberLengths:[NSIndexSet indexSetWithIndex:14]
                                                        validCvvLength:3
                                                          formatSpaces:nil];

        BTKCardType *maestro = [[BTKCardType alloc] initWithBrand:BTKLocalizedString(CARD_TYPE_MAESTRO)
                                                           prefixes:@[@"5018", @"5020", @"5038", @"6304", @"6759", @"6761", @"6762", @"6763"]
                                                 validNumberLengths:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(12, 8)]
                                                     validCvvLength:3
                                                       formatSpaces:nil];

        BTKCardType *unionPay = [[BTKCardType alloc] initWithBrand:BTKLocalizedString(CARD_TYPE_UNION_PAY)
                                                            prefixes:@[@"62"]
...
+ (NSDictionary *)cardsByBrand {
    ...
    dispatch_once(&p, ^{
        ...
        for (BTKCardType *cardType in [self allCards]) {
...
- (NSAttributedString *)formatNumber:(NSString *)input kerning:(CGFloat)kerning{
    ...
    input = [BTKUtil stripNonDigits:input];
...
- (BOOL)validAndNecessarilyCompleteNumber:(NSString *)number {
    return (number.length == self.validNumberLengths.lastIndex &&
            ([BTKUtil luhnValid:number] || [self.brand isEqualToString:BTKLocalizedString(CARD_TYPE_UNION_PAY)]));
}
