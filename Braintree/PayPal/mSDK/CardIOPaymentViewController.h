...
/// @param key key into *.strings file.
/// @param languageOrLocale See property languageOrLocale for full discussion.
/// @param adaptedForCountry Country code for adaptation (if nil, will use current device locale country)
/// @param bundle The bundle containing the *.strings files. If nil, defaults to [[CardIOBundle sharedInstance] NSBundle].
/// @param showMissingKeyAlert If key is not found in *.strings file, display an alert to aid in debugging.
NSString *CardIOLocalizedStringExternalWithAlert(NSString *key,
