...
#pragma mark - Internal environment setup

/// Add a custom-defined environment to the list of available environments.
/// @param environments Custom environments to add.
+ (void)addEnvironments:(nonnull NSDictionary *)environments;
...
+ (nonnull NSDictionary *)availableEnvironments;

// @note For changing the clientId at runtime; currently used only by the Demo app.
+ (void)setClientId:(nonnull NSString *)clientId forEnvironment:(nonnull NSString *)environment;
