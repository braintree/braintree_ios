                   ...
                   transactionAmount:amount
                          completion:^(BTThreeDSecureLookupResult *lookupResult, NSError *error) {
                              ...
                              if (lookupResult.requiresUserAuthentication) {
                                  ...
                                  [self.analyticsClient postAnalyticsEvent:@"ios.threedsecure.authentication-start"];
                              } else {
...
{
    [self.apiClient fetchOrReturnRemoteConfiguration:^(BTJSON * __nullable remoteConfiguration, NSError * __nullable error) {
        NSMutableDictionary *requestParameters = [@{ @"amount": amount } mutableCopy];

        if (remoteConfiguration[@"merchantAccountId"]) {
            requestParameters[@"merchant_account_id"] = remoteConfiguration[@"merchantAccountId"];
        }
                        ...
                        completion:(void (^)(BTThreeDSecureViewControllerCompletionStatus))completionBlock
{
    ...
    [self.analyticsClient postAnalyticsEvent:@"ios.threedsecure.authenticated"];
}

- (void)threeDSecureViewController:(__unused BTThreeDSecureAuthenticationViewController *)viewController
                  didFailWithError:(NSError *)error {
    if ([error.domain isEqualToString:BTThreeDSecureErrorDomain] && error.code == BTThreeDSecureErrorCodeFailedAuthentication) {
        [self.analyticsClient postAnalyticsEvent:@"ios.threedsecure.error.auth-failure"];
    } else {
        [self.analyticsClient postAnalyticsEvent:@"ios.threedsecure.error.unrecognized-error"];
    }
...
- (void)threeDSecureViewControllerDidFinish:(BTThreeDSecureAuthenticationViewController *)viewController {
    if (self.upgradedTokenizedCard) {
        ...
        [self.analyticsClient postAnalyticsEvent:@"ios.threedsecure.canceled"];
    }
...
- (void)threeDSecureViewController:(__unused BTThreeDSecureAuthenticationViewController *)viewController
    ...
    [self.analyticsClient postAnalyticsEvent:[NSString stringWithFormat:@"ios.threedsecure.error.webview-error.%@", request.URL.host]];
}
