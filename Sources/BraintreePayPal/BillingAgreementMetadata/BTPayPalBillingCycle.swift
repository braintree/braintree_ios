...
public struct BTPayPalBillingCycle {
    ...
    ///
    /// For example, if the `billingInterval` is DAY with an `billingIntervalCount` of 2, the subscription is billed once every two days. Maximum values {DAY -> 365}, {WEEK, 52}, {MONTH, 12}, {YEAR, 1}.
    let billingIntervalCount: Int
    
    /// The number of times this billing cycle gets executed.
    ///
    /// Trial billing cycles can only be executed a finite number of times (value between 1 and 999). Regular billing cycles can be executed infinite times (value of 0) or a finite number of times (value between 1 and 999).
    let numberOfExecutions: Int
    
    /// The sequence of the billing cycle.
    ///
    /// Starting value 1 and max value 100. All billing cycles should have unique sequence values.
    let sequence: Int?
    
    /// The date and time when the billing cycle starts, in Internet date and time format `YYYY-MM-DDT00:00:00Z`.
    ///
