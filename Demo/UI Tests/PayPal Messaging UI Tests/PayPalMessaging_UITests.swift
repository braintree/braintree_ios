...
final class PayPalMessaging_Success_UITests: XCTestCase {
    ...
    func testStart_withValidRequest_firesDelegates() {
        XCTAssertTrue(app.buttons["DELEGATE: didAppear fired"].waitForExistence(timeout: 20))
        ...
        XCTAssertTrue(app.buttons["DELEGATE: didSelect fired"].waitForExistence(timeout: 20))
    }
}
