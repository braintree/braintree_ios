...
class ThreeDSecure_V2_UITests: XCTestCase {
    ...
    func testThreeDSecurePaymentFlowV2_challengeFlow_andTransacts() {
        ...
        let textField = app.textFields.allElementsBoundByIndex.first { $0.isHittable } ?? app.secureTextFields.allElementsBoundByIndex.first { $0.isHittable } ?? app.textFields.element(boundBy: 0)
