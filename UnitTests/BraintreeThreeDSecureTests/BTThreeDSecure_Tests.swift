...
class BTThreeDSecure_UnitTests: XCTestCase {
    ...
    func testStartPayment_successfulResult_callsCompletionBlock() {
        ...
        let webAuthSession = MockBTWebAuthenticationSession()
        let driver = BTPaymentFlowDriver(apiClient: mockAPIClient, webAuthenticationSession: webAuthSession)

        mockAPIClient.cannedResponseBody = BTJSON(value: getAuthRequiredLookupResponse())

        let paymentFinishedExpectation = self.expectation(description: "Start payment expectation")
        driver.startPaymentFlow(threeDSecureRequest) { (result, error) in
            XCTAssertNil(error)
