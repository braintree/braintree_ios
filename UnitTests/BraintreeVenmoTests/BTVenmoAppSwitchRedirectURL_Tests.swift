...
class BTVenmoAppSwitchRedirectURL_Tests: XCTestCase {
    ...
    func testAppSwitchURL_whenAllValuesAreInitialized_returnsURLWithPaymentContextID() {
        do {
            ...
            XCTAssertTrue(requestURL.appSwitchLink().absoluteString.contains("com.venmo.touch.v2://x-callback-url/vzero/auth"))

            let components = URLComponents(string: requestURL.appSwitchLink().absoluteString)
    ...
    func testUniversalLinkURL_whenAllValuesInitialized_returnsURLWithAllValues() {
        do {
            ...
            XCTAssertTrue(requestURL.universalLink().absoluteString.contains("https://venmo.com/go/checkout"))

            let components = URLComponents(string: requestURL.universalLink().absoluteString)
            guard let queryItems = components?.queryItems else { XCTFail(); return }
