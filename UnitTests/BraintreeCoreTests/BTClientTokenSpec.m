...
- (void)testInitialization_whenVersionIsUnsupported_returnsError {
    NSError *error;
    BTClientToken *clientToken = [[BTClientToken alloc] initWithClientToken:[TestClientTokenFactory tokenWithVersion:2 overrides:@{ @"version": @0 }] error:&error];
...
- (void)testCopy_returnsADifferentInstance {
    NSString *clientTokenRawJSON = [TestClientTokenFactory tokenWithVersion:2];
