...
final class BTPayPalSavedPaymentMethodViewModel_Tests: XCTestCase {
    ...
    /// `onAppear` refires on tab switches and navigation pop-backs, so the fetch it issues is
    /// asserted here. The post-edit half of the guard needs `didCompleteEdit`, which only a real
    /// edit can set, so it is not reachable until `BTPayPalClient` is injectable.
    func testOnAppear_beforeAnyEdit_fetchesTheStickyFI() async {
        mockAPIClient.cannedResponseBody = Self.instrumentResponse()
        let sut = makeSUT()
